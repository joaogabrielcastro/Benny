...
class GatewayController {
  ...
  async baixarCertificado(req, res) {
    try {
      ...
      if (!cfg) return res.status(404).json({ error: 'Config not found' });
      if (!cfg.certificado_a1) return res.status(404).json({ error: 'Certificado não encontrado' });

      // Registrar auditoria simples
      const usuario = req.headers['x-audit-user'] || req.query.user || 'unknown';
      await gatewayConfigsService.registrarAuditoria(id, usuario);

      // Decrypt certificado and return base64
      import('../lib/crypto.js').then(({ decryptToBase64 }) => {
        try {
          const base64 = decryptToBase64(cfg.certificado_a1);
          res.json({ certificadoBase64: base64 });
        } catch (e) {
          res.status(500).json({ error: 'Erro ao descriptografar certificado' });
        }
      }).catch(err => {
