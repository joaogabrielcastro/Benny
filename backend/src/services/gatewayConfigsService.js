...
class GatewayConfigsService {
  ...
  async listar() {
    const res = await pool.query("SELECT id, empresa_id, provider, ativo, criado_em FROM gateway_configs ORDER BY id DESC");
    return res.rows;
  }

  async buscarPorId(id) {
    const res = await pool.query("SELECT * FROM gateway_configs WHERE id = $1", [id]);
    return res.rows[0];
  }
